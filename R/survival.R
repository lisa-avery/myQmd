# survival.R
# Pre-computation of survival endpoints (age, OS, PFS, RFS) and a
# Kaplan-Meier summary table with median and timepoint survival (log-log CIs).
#
# Suggests packages: survival, dplyr, reportRmd, lubridate

#' Days per unit for a time unit
#'
#' @param time_unit One of "days", "months", or "years".
#' @return Numeric divisor (days per unit).
#' @export
...
#' Coerce a column to Date, parsing if necessary
#'
#' Returns Date/POSIXct input as Date. For other input it uses a project date
#' parser if one is on the search path (`parse_date_column()` or
#' `convert_column_to_date()` from data_import.R), otherwise falls back to
#' lubridate then base parsing.
#'
#' @param x A vector of dates (Date, POSIXct, character, or numeric).
...
safe_date <- function(x) {
  ...
  # BUGFIX: original sourced an external deid_functions.R on load and assumed
  # parse_date_column() existed. Use whichever parser is available, else parse
  # locally so the script is self-contained.
  if (exists("parse_date_column", mode = "function")) {
    return(get("parse_date_column")(x))
  }
  if (exists("convert_column_to_date", mode = "function")) {
    return(get("convert_column_to_date")(x))
  }
...
#' Compute age at a reference date
#'
#' @param df A data frame.
#' @param id Name of the ID column.
#' @param dob Name of the date-of-birth column.
#' @param ref_date Name of the reference date column (age computed at this date).
#' @return A data frame with the id column and `age_years`.
#' @export
...
#' Compute overall survival time and status
#'
#' @param df A data frame.
#' @param id Name of the ID column.
#' @param start Name of the start (time-origin) date column, e.g. diagnosis.
#' @param event Name of the event date column (e.g. death; NA if none).
#' @param censor Name of the censor date column (e.g. last follow-up).
#' @param time_unit "days", "months", or "years".
#' @return A data frame with id, `os_time_{unit}` and `os_status` (1 = event).
#' @export
...
#' Compute progression-free survival time and status
#'
#' The event column should hold the earliest of progression or death,
#' e.g. `df$pfs_event_date <- pmin(progression, death, na.rm = TRUE)`.
#'
#' @param df A data frame.
#' @param id Name of the ID column.
#' @param start Name of the start (time-origin) date column.
#' @param event Name of the event date column (earliest of progression/death).
#' @param censor Name of the censor date column.
#' @param time_unit "days", "months", or "years".
#' @return A data frame with id, `pfs_time_{unit}` and `pfs_status`.
#' @export
...
#' Compute relapse-free survival time and status
#'
#' Time origin is end of treatment; the event column should hold the earliest
#' of relapse or death. Patients without a start (EOT) date return NA.
#'
#' @param df A data frame.
#' @param id Name of the ID column.
#' @param start Name of the end-of-treatment date column.
#' @param event Name of the event date column (earliest of relapse/death).
#' @param censor Name of the censor date column.
#' @param time_unit "days", "months", or "years".
#' @return A data frame with id, `rfs_time_{unit}` and `rfs_status`.
#' @export
...
summarise_survival <- function(
  ...
  surv_formula <- as.formula(
    paste0("survival::Surv(", time, ", ", status, ") ~ ", group)
  )

  # Fit Kaplan-Meier with log-log CIs
  fit <- survival::survfit(surv_formula, data = data, conf.type = "log-log")
  ...
  if (overall) {
    surv_formula_overall <- as.formula(
      paste0("survival::Surv(", time, ", ", status, ") ~ 1")
    )
    fit_overall <- survival::survfit(
