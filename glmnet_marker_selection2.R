...
glmnet_marker_selection <- function(
  ...
  # Check for factor candidate markers
  factor_markers <- candidate_markers[
    sapply(df[, candidate_markers, drop = FALSE], is.factor)
  ]
  if (length(factor_markers) > 0) {
    stop(
      "Factor candidate markers are not supported. ",
      "Please convert to numeric or use dummy variables: ",
  ...
  if (n_obs <= length(candidate_markers)) {
    stop(
      "Sample size (n = ",
      n_obs,
      ") must exceed the number of candidate markers (p = ",
      length(candidate_markers),
      "). Elastic net LOO is not appropriate when n <= p."
    )
  }
  ...
  if (length(transformed_markers) > 0 && interactive()) {
    message(
      "The following markers were transformed: ",
  ...
  select_lambda <- function(x, y, pf) {
    ...
    if (n_nonzero == 0) {
      ...
      if (n_nonzero == 0) {
        warning(
          "AIC also selected an intercept-only model. Results may not be informative."
        )
      }
    }
  ...
  if (poscat != "1") {
    poscat_desc <- paste0(
      "The positive outcome category was '",
      poscat,
      "'. "
    )
  } else {
  ...
  if (has_missing) {
    mi_desc <- paste0(
      "Residual missing values among candidate markers were imputed using predictive ",
      "mean matching via the mice algorithm to generate ",
      n_imputations,
